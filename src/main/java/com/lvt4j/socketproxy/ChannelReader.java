...
public class ChannelReader extends Thread implements UncaughtExceptionHandler {
    ...
    public void readUntilByte(SocketChannel channel, byte specifyByte
        ...
        registerQueue.add(() -> {
            ReadUntilSpecifyByteMeta meta = new ReadUntilSpecifyByteMeta();
            meta.channel = channel;
    ...
    private abstract static class ReadMeta {
     ...
     * 只读一个字节则结束
     */
    private static class ReadOneMeta extends ReadMeta {
     ...
     * 读到特定字节则结束
     */
    private static class ReadUntilSpecifyByteMeta extends ReadMeta {
     ...
     * 读够特定长度，则结束
     */
    private static class ReadLengthMeta extends ReadMeta {
        private IOExceptionConsumer<byte[]> onRead;
     ...
     * 读任意长度（不能为0），则结束
     */
    private static class ReadAnyMeta extends ReadMeta {

    ...
    private class ServerMeta {
        ...
        public void cleanIdle() {
            synchronized (connections) {
                for (ConnectMeta connect : ImmutableSet.copyOf(connections)) {
                    if (System.currentTimeMillis() - connect.latestTouchTime < TcpService.this.config.getMaxIdleTime())
                        continue;
                    connect.destory();
                }
            }
        ...
        private class ConnectMeta {
            ...
            private Map<String, String> pws_headers(URI pwsServer) {
                ...
                if (pwsServer.getUserInfo() != null)
                    headers.put(AUTHORIZATION, "Basic " + Base64.getEncoder().encodeToString(pwsServer.getUserInfo().getBytes()));
                return headers;
            }

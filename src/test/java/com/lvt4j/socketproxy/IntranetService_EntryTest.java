...
public class IntranetService_EntryTest extends BaseTest {
    ...
    private void trans(Socket relay, int idBase) throws Throwable {
        ...
        client1SendData = EMPTY_BYTE_ARRAY;
        client1SendLengths = new LinkedList<>();
        ...
        client2SendData = EMPTY_BYTE_ARRAY;
        client2SendLengths = new LinkedList<>();
        ...
        Thread entryHeartbeatSender = new Thread("entryHeartbeatSender") {
            @SneakyThrows
            public void run() {
                for (int i = 0; i < count; i++) {
        ...
        Thread relay1Sender = new Thread("relay1Sender") {
            @SneakyThrows
            public void run() {
                for (int i = 0; i < count; i++) {
                    byte[] data = rand();
        ...
        Thread relay2Sender = new Thread("relay2Sender") {
            @SneakyThrows
            public void run() {
                for (int i = 0; i < count; i++) {
                    byte[] data = rand();
        ...
        Thread relayHeartbeatSender = new Thread("relayHeartbeatSender") {
            @SneakyThrows
            public void run() {
                for (int i = 0; i < count; i++) {
                    synchronized (config) { //relay 互斥
        ...
        relayReceiveData1 = EMPTY_BYTE_ARRAY;
        relayReceiveLengths1 = new LinkedList<>();
        relayReceiveData2 = EMPTY_BYTE_ARRAY;
        relayReceiveLengths2 = new LinkedList<>();
        Thread relayReceiver = new Thread("relayReceiver") {
            @SneakyThrows
            public void run() {
        ...
        relay1Sender.start();
        relay2Sender.start();
        relayHeartbeatSender.start();
        relayReceiver.start();

        client1Sender.join();
        client2Sender.join();
        entryHeartbeatSender.join();
        relay1Sender.join();
        relay2Sender.join();
        relayHeartbeatSender.join();
        relayReceiver.join();
